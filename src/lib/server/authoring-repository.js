...
export class AuthoringRepositoryPathError extends Error {
  /**
   * @param {string} message
   */
  constructor(message) {
    super(message);
    this.name = 'AuthoringRepositoryPathError';
    this.code = 'AUTHORING_PATH_INVALID';
  }
}

export class AuthoringRevisionConflictError extends Error {
  /**
   * @param {string} relativePath
   * @param {string | null} expectedRevision
   * @param {string | null} actualRevision
   */
  constructor(relativePath, expectedRevision, actualRevision) {
    super(`Stale authoring revision for ${relativePath}.`);
    this.name = 'AuthoringRevisionConflictError';
    this.code = 'AUTHORING_REVISION_CONFLICT';
...
/**
 * Normalize an authoring path without permitting callers to escape the project.
 *
 * The contract deliberately rejects `..` segments instead of merely resolving
 * them back inside the root. Browser- or adapter-supplied paths must remain
 * explicit project-relative paths.
 *
 * @param {string} relativePath
 * @returns {string}
 */
export function normalizeAuthoringPath(relativePath) {
  if (typeof relativePath !== 'string' || relativePath.length === 0) {
    throw new AuthoringRepositoryPathError('Authoring path must be a non-empty string.');
  }

  if (relativePath.includes('\0')) {
    throw new AuthoringRepositoryPathError('Authoring path must not contain NUL bytes.');
  }
  ...
  if (
    ...
    throw new AuthoringRepositoryPathError('Authoring path must be project-relative.');
  }
  ...
  if (!normalized) {
    throw new AuthoringRepositoryPathError('Authoring path must identify a project file.');
  }
