...
| `EditorialText` | `later-after-inversion` | Atelier Mark parser/resolvers/classes and site color tokens | dynamic tag, raw-markup parsing/fallback, HTML safety and token behavior | component runtime, SSR, injection cases, every syntax class, fallback, visual | first separate Atelier Mark ownership; then inject a renderer/snippet or consume a sibling package | high boundary risk; current component is an Atelier-Kit adapter over format-specific behavior, not yet a `giadaware-ui-components` primitive |
