...
const sharpDependency =
  kitPackage.dependencies[sharpPackage];
...
/** @param {string} clientRoot @param {string | undefined} value */
function setYamlDependency(clientRoot, value) {
  const packagePath =
    path.join(clientRoot, 'package.json');
  const packageJson =
    JSON.parse(
...
/** @param {string} clientRoot */
function provideInstalledRuntimeDependenciesForPortableRunner(clientRoot) {
...
test('successfully migrates the fresh issue-232 dependency, identity and artifact', async () => {
  ...
  try {
    ...
    assert.equal(
      clientPackage.dependencies[sharpPackage],
      sharpDependency
    );
...
for (const [label, value] of [
  ...
  test(`preserved package.json with ${label} YAML dependency aborts with zero mutations`, async () => {
    const clientRoot = makeClient();
...
for (const [label, value] of [
  ...
  test(`preserved package.json with ${label} sharp dependency aborts with zero mutations`, async () => {
    const clientRoot = makeClient();
    ...
    try {
      ...
      await assert.rejects(
        runMain(clientRoot),
        new RegExp(
          `preserves package\\.json.*dependencies\\.sharp.*Expected "${sharpDependency.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}".*Remove the package\\.json preserve rule`
        )
      );
...
test('sharp runtime dependency planner follows the Kit package contract', () => {
  const clientRoot = makeClient();
...
test('YAML dependency migration is visible in dry-run and makes no mutations', async () => {
  const clientRoot = makeClient();
...
test('upgrade installs the existing-file test runner without requiring a root test directory', async () => {
  ...
  try {
    ...
    provideInstalledRuntimeDependenciesForPortableRunner(clientRoot);
