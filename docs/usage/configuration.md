...
- `language` sets `<html lang="…">`, the studio / CLI operator UI locale, **and** visitor-facing UI labels (`en`, `it`, `it-IT`, etc.) where supported. English is the canonical source for Atelier-Kit-owned presentation strings; unsupported or missing UI localization falls back to English. Client-authored YAML content (item titles, about text, news bodies) remains authoritative and is not silently translated.
